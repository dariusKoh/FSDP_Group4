...
function Overview({ testLogs, proj_id, projName, lastDate = "11/12/2024", onClose, onTestCaseClick }) {
        ...
        <table>
          ...
          <tbody>
            {filteredLogs.map((log, index) => (
              <tr 
                key={index} 
                className={log.status === "PASSED" ? "pass" : "fail"}
                onClick={() => onTestCaseClick("TestCaseDetails", null, log)}
              ...
              >
                ...
                <td>{log.status}</td>
                <td>{log.duration}</td>
                <td>{log.failureMessages?.slice(0, 50) || "N/A"}...</td>
